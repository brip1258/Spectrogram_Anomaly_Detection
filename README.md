...
These results are promising, and the increase in scores suggest the model choice is on the right track. Though there may not have been a large increase in the performance, the initial sample size used in the analysis can be adjusted to improve model performance. Even with this approach it would be conditional due to the imbalance in the original data set.

For future improvements we can take steps by obtaining more data or I could create augmented/synthetic data. I could also, modify the model architecture, utilize different epoch values, or adjust different hyper parameters. Another aspect would to be to introduce another feature that was represented in the data, such as band frequency. I could also simplify the problem if allowed to a binary classification system of normal and not normal, the performance could be significantly better and you may be able to utilize a lighter weight model. 
